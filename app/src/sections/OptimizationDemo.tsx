...
const OptimizationDemo = () => {
  ...
  const handleOptimize = async () => {
    ...
    try {
      ...
      // 根据选择的优化模式调整参数
      let modeParams: { max_iter: number; use_dqn: boolean } = { max_iter: maxIter[0], use_dqn: useDqn };
      ...
      console.log('优化结果类型:', typeof result);
      console.log('优化结果是否为对象:', typeof result === 'object' && result !== null);
      if (result) {
        console.log('设置optimizationResult:', result);
        console.log('result是否有metrics属性:', 'metrics' in result);
        console.log('result.metrics:', result.metrics);
        console.log('result是否有module1_output属性:', 'module1_output' in result);
        console.log('result是否有module2_output属性:', 'module2_output' in result);
        console.log('result是否有module3_output属性:', 'module3_output' in result);
  ...
  const costBreakdownData = () => {
    try {
      ...
      const breakdown = optimizationResult?.module3_output?.total_cost_summary?.cost_breakdown || 
                       optimizationResult?.total_cost_summary?.cost_breakdown || 
                       optimizationResult?.cost_breakdown;
      if (!breakdown) return [];
      
      const totalCost = optimizationResult?.metrics?.total_cost || 
                       optimizationResult?.module3_output?.total_cost_summary?.total_cost || 1;
      ...
      const data = Object.entries(breakdown).map(([name, value], index) => ({
        ...
        percentage: ((Number(value) || 0) / totalCost) * 100,
        color: COLORS[index % COLORS.length]
      }));
      
      // 添加土建成本和运维成本（如果不存在）
      const civilCost = Number(optimizationResult?.metrics?.civil_cost || 0);
      if (!data.some(item => item.name === '土建成本') && civilCost > 0) {
        data.push({
          name: '土建成本',
          value: civilCost,
          percentage: (civilCost / totalCost) * 100,
          color: COLORS[data.length % COLORS.length]
        });
      }
      
      const operationCost = Number(optimizationResult?.metrics?.operation_cost || 0);
      if (!data.some(item => item.name === '运维成本') && operationCost > 0) {
        data.push({
          name: '运维成本',
          value: operationCost,
          percentage: (operationCost / totalCost) * 100,
          color: COLORS[data.length % COLORS.length]
        });
      }
  ...
  const zoneDistributionData = () => {
    try {
      ...
      if (!zoneList || !Array.isArray(zoneList)) {
        // 如果没有分区数据，返回默认数据
        return [
      ...
      return zoneList.map((zone: any) => ({
        ...
        panels: Number(zone.pva_count) || 0,
        power: Number(zone.total_power) || 0
      }));
    } catch (error) {
  ...
  const zonePowerHeatmapData = () => {
    try {
      ...
      return zoneList.map((zone: any, index: number) => ({
        x: index % 3,
        y: Math.floor(index / 3),
        value: Number(zone.total_power) || 0,
        zone_id: zone.zone_id || index
      }));
    } catch (error) {
      console.error('处理分区热力图数据时出错:', error);
      // 出错时返回默认数据
      return [
        { x: 0, y: 0, value: 10.5 },
        { x: 1, y: 0, value: 10.5 },
  ...
  const cableRouteCostData = () => {
    try {
      // 检查不同的数据结构
      const routeList = optimizationResult?.module2_output?.cable_routes;
      
      if (!routeList || !Array.isArray(routeList)) {
        // 如果没有电缆路由数据，返回默认数据
        return [
          { name: '电缆 1', length: 22.36, cost: 1.12 },
          { name: '电缆 2', length: 33.54, cost: 1.68 },
          { name: '电缆 3', length: 45.21, cost: 2.26 },
          { name: '电缆 4', length: 18.75, cost: 0.94 }
        ];
      }
      
      return routeList.map((route: any, index: number) => ({
        name: route.cable_id || `电缆 ${index + 1}`,
        length: route.length || 0,
        cost: route.cost || 0
      }));
    } catch (error) {
      console.error('处理电缆路由成本数据时出错:', error);
      // 出错时返回默认数据
      return [
        { name: '电缆 1', length: 22.36, cost: 1.12 },
        { name: '电缆 2', length: 33.54, cost: 1.68 },
  ...
  const efficiencyReliabilityData = () => {
    try {
      // 检查不同的数据结构
      const paretoData = optimizationResult?.module3_output?.pareto_front;
      
      if (!paretoData || !Array.isArray(paretoData)) {
        // 如果没有帕累托前沿数据，返回默认数据
        return [
          { efficiency: 0.85, reliability: 1.1 },
          { efficiency: 0.88, reliability: 1.2 },
          { efficiency: 0.91, reliability: 1.3 },
          { efficiency: 0.94, reliability: 1.4 },
          { efficiency: 0.97, reliability: 1.5 }
        ];
      }
      
      return paretoData.map((point: any) => ({
        efficiency: point.efficiency || 0,
        reliability: point.reliability || 0,
        cost: point.cost || 0
      }));
    } catch (error) {
      console.error('处理效率与可靠性关系数据时出错:', error);
      // 出错时返回默认数据
      return [
        { efficiency: 0.85, reliability: 1.1 },
        { efficiency: 0.88, reliability: 1.2 },
  ...
  const lifecycleCostData = () => {
    try {
      ...
      const initialCost = optimizationResult?.metrics?.total_cost || 150;
      const operationCost = optimizationResult?.metrics?.operation_cost || 1;
      
      return years.map(year => ({
        year,
        initial_cost: year === 1 ? initialCost : 0,
        operation_cost: operationCost,
        maintenance_cost: 0.5 + year * 0.02,
        total_cost: year === 1 ? initialCost + operationCost + 0.5 : operationCost + 0.5 + year * 0.02
      }));
    } catch (error) {
      console.error('处理全生命周期成本数据时出错:', error);
      // 出错时返回默认数据
      const years = Array.from({ length: 25 }, (_, i) => i + 1);
      return years.map(year => ({
        year,
        initial_cost: year === 1 ? 150 : 0,
        operation_cost: 1,
        maintenance_cost: 0.5 + year * 0.02,
        total_cost: year === 1 ? 151.5 : 1.5 + year * 0.02
      }));
    }
  };
  ...
  const trenchData = () => {
    try {
      ...
      if (!trenchList || !Array.isArray(trenchList)) {
        // 如果没有管沟数据，返回默认数据
        return [
  ...
  const cableRouteData = () => {
    try {
      ...
      return routeList.map((route: any) => ({
        cable_id: route.cable_id || `cable_${Math.floor(Math.random() * 1000)}`,
        path: route.path || [],
        length: route.length || 0,
        cost: route.cost || 0
      }));
    } catch (error) {
      console.error('处理电缆路由数据时出错:', error);
      // 出错时返回默认数据
      return [
        {
          cable_id: 'cable_1',
  ...
  const performanceRadarData = () => {
    try {
      // 获取成本数据
      const totalCost = optimizationResult?.metrics?.total_cost || 
                       optimizationResult?.module3_output?.total_cost_summary?.total_cost || 150;
      
      // 改进成本效益计算方式，使其更加合理
      // 假设成本在 100-200 万之间，映射到 70-90 分
      const costScore = Math.max(0, Math.min(100, 90 - (totalCost - 100) * 0.2));
      
      // 系统效率：通常在 0.9-1.0 之间，映射到 80-95 分
      const efficiency = optimizationResult?.metrics?.efficiency || 0.92;
      const efficiencyScore = Math.max(0, Math.min(100, 80 + (efficiency - 0.9) * 150));
      
      // 系统可靠性：通常在 1.0-1.6 之间，映射到 70-85 分
      const reliability = optimizationResult?.metrics?.reliability || 1.2;
      const reliabilityScore = Math.max(0, Math.min(100, 70 + (reliability - 1.0) * 25));
      ...
      return [
        ...
        {
          subject: '覆盖面积利用率',
          A: optimizationResult?.metrics?.coverage_rate || 90,
          fullMark: 100
        },
        {
          subject: '共沟成本优化率',
        ...
        {
          subject: '约束满足度',
          A: optimizationResult?.metrics?.constraint_satisfaction || 90,
          fullMark: 100
        },
        {
          subject: '成本效益',
      ...
      // 返回默认数据，确保图表能够正常显示
      return [
        { subject: '系统效率', A: 85, fullMark: 100 },
        { subject: '系统可靠性', A: 75, fullMark: 100 },
        { subject: '覆盖面积利用率', A: 90, fullMark: 100 },
        { subject: '共沟成本优化率', A: 66, fullMark: 100 },
        { subject: '约束满足度', A: 90, fullMark: 100 },
        { subject: '成本效益', A: 80, fullMark: 100 }
      ];
    }
  };
  ...
  const paretoFrontData = () => {
    ...
    try {
      return paretoData.map((point: any, index: number) => ({
        id: index,
        cost: point.cost || 0,
        efficiency: point.efficiency || 0,
        reliability: point.reliability || 0
          ...
          {!optimizationResult ? (
            ...
            >
              <motion.div
                ...
                className="mb-6"
              >
                <Database className="w-24 h-24 mx-auto opacity-30" />
              </motion.div>
              <motion.p 
              ...
              <p className="text-sm text-gray-500">优化结果将在此处显示</p>
              <motion.div 
                className="mt-8 w-64 h-1 bg-white/10 rounded-full"
              >
                <motion.div 
                  className="h-1 bg-gradient-to-r from-cyan-400 to-emerald-500 rounded-full"
                  initial={{ width: 0 }}
                  animate={{ width: "100%" }}
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                      ...
                      <div className="grid grid-cols-1 gap-3 mt-6">
                        {costBreakdownData().map((item, index) => (
                          ...
                          >
                            ...
                            <span className={`ml-2 text-xs text-gray-400`}>{typeof item.percentage === 'number' ? item.percentage.toFixed(1) : '0.0'}%</span>
              ...
              <TabsContent value="module1">
                <div className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="p-4 bg-white/5 rounded-lg border border-white/10">
                      <h4 className="text-sm font-medium text-white mb-4 flex items-center gap-2">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#00d4ff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect><line x1="3" y1="9" x2="21" y2="9"></line><line x1="9" y1="21" x2="9" y2="9"></line></svg>
                        分区功率分布热力图
                      </h4>
                      <div className="h-72">
                        <ResponsiveContainer width="100%" height="100%">
                          <BarChart data={zonePowerHeatmapData()} margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
                            <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.1)" />
                            <XAxis dataKey="x" stroke="#6b7280" fontSize={12} label={{ value: 'X 坐标', position: 'insideBottomRight', offset: -10, fill: '#6b7280' }} />
                            <YAxis dataKey="y" stroke="#6b7280" fontSize={12} label={{ value: 'Y 坐标', angle: -90, position: 'insideLeft', fill: '#6b7280' }} />
                            <Tooltip 
                              contentStyle={{ 
                                background: 'rgba(10, 15, 26, 0.95)', 
                                border: '1px solid rgba(0, 212, 255, 0.3)',
                              ...
                              formatter={(value: any, name: any) => [`${value.toFixed(2)} kW`, '功率']}
                              labelFormatter={(label, payload) => `分区 ${payload[0].payload.zone_id}`}
                            />
                            <Bar 
                              dataKey="value" 
                            ...
                            <defs>
                              <linearGradient id="heatmapGradient" x1="0" y1="0" x2="0" y2="1">
                                ...
                                <stop offset="95%" stopColor="#06d6a0" stopOpacity={0.5}/>
                              </linearGradient>
                            </defs>
                          </BarChart>
                        </ResponsiveContainer>
                  ...
                  <div className="p-4 bg-white/5 rounded-lg border border-white/10">
                    ...
                    <div className="flex justify-center">
                      ...
                      >
                        <img
                          src={`/algorithm/static/partition_${currentInstanceId}.png`}
                          alt="分区效果图" 
                          className="max-w-full h-auto rounded-lg shadow-lg"
                          onError={(e) => {
                            (e.target as HTMLImageElement).src = `/algorithm/static/partition_r1.png`;
                          }}
                        />
                        <div className="absolute top-4 right-4 flex gap-2">
                          <motion.button
                            ...
                            title="放大"
                          >
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8"></circle><line x1="21" y1="21" x2="16.65" y2="16.65"></line></svg>
                          </motion.button>
                          <motion.button
                            whileHover={{ scale: 1.1 }}
                            whileTap={{ scale: 0.9 }}
                          ...
                          <motion.button
                            ...
                            title="重置"
                          >
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21.5 2v6h-6M2.5 22v-6h6M2 11.5a10 10 0 0 1 18.8-4.3M22 12.5a10 10 0 0 1-18.8 4.2"></path></svg>
                          </motion.button>
                        </div>
                        <div className="absolute bottom-4 left-4 bg-black/50 rounded-lg p-2 text-xs text-white">
                          分区数: {optimizationResult?.module1_output?.zone_summary?.length || 0}
                        </div>
                      </motion.div>
                    </div>
                    <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-3">
                      {optimizationResult?.module1_output?.zone_summary?.map((zone: any, index: number) => (
                        <motion.div
                          ...
                          className="p-3 bg-white/5 rounded-lg border border-white/10"
                        >
                          <div className="text-xs text-gray-400">分区 {zone.zone_id || index + 1}</div>
                          <div className="text-lg font-bold text-cyan-400">{zone.pva_count || 0} 面板</div>
                          <div className="text-xs text-gray-400">周长: {zone.perimeter || 0}m</div>
                          <div className="text-xs text-gray-400">功率: {(zone.total_power || 0).toFixed(2)}kW</div>
                        </motion.div>
                      ))}
                    </div>
                  </div>
              ...
              <TabsContent value="module2">
                <div className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    >
                      ...
                      <div className="h-72">
                        <ResponsiveContainer width="100%" height="100%">
                          <BarChart 
                            data={cableRouteCostData().slice(0, 10)} // 只显示前10条电缆
                            margin={{ top: 20, right: 30, left: 20, bottom: 60 }}
                          >
                            <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.1)" />
                            <XAxis 
                              ...
                              textAnchor="end"
                              height={60}
                              label={{ value: '电缆ID', position: 'insideBottomRight', offset: -10, fill: '#6b7280' }}
                            />
                            <YAxis stroke="#6b7280" fontSize={12} label={{ value: '数值', angle: -90, position: 'insideLeft', fill: '#6b7280' }} />
                            <Tooltip 
                              contentStyle={{ 
                    ...
                    >
                      ...
                      <div className="h-72">
                        <ResponsiveContainer width="100%" height="100%">
                          <RechartsLineChart 
                            data={efficiencyReliabilityData()}
                          ...
                          >
                            ...
                            <XAxis 
                              ...
                              stroke="#6b7280" 
                              fontSize={12}
                              label={{ value: '系统效率', position: 'insideBottom', offset: -5, fill: '#6b7280' }}
                            />
                            <YAxis 
                              dataKey="reliability" 
                              stroke="#6b7280" 
                              fontSize={12}
                              label={{ value: '系统可靠性', angle: -90, position: 'insideLeft', fill: '#6b7280' }}
                            />
                            <Tooltip 
                              contentStyle={{ 
                                background: 'rgba(10, 15, 26, 0.95)', 
                  ...
                  <div className="p-6 bg-white/5 rounded-xl border border-white/10 hover-lift">
                    <motion.h4 
                      className="text-lg font-semibold text-white mb-6 flex items-center gap-3"
                      initial={{ opacity: 0, x: -20 }}
                      animate={{ opacity: 1, x: 0 }}
                    ...
                    <div className="h-96">
                      <ResponsiveContainer width="100%" height="100%">
                        <RechartsLineChart 
                          data={cableRouteData().flatMap(route => 
                            route.path.map((point: any, index: number) => ({
                              name: `${route.cable_id}_${index}`,
                              x: point[0],
                              y: point[1],
                        ...
                        >
                          ...
                          <XAxis 
                            ...
                            stroke="#6b7280" 
                            fontSize={12}
                            label={{ value: 'X 坐标 (m)', position: 'insideBottom', offset: -5, fill: '#6b7280' }}
                          />
                          <YAxis 
                            dataKey="y" 
                            stroke="#6b7280" 
                            fontSize={12}
                            label={{ value: 'Y 坐标 (m)', angle: -90, position: 'insideLeft', fill: '#6b7280' }}
                          />
                          <Tooltip 
                            contentStyle={{ 
                              background: 'rgba(10, 15, 26, 0.95)', 
                            ...
                            formatter={(value: any, name: any) => [`${value.toFixed(2)} m`, name === 'x' ? 'X坐标' : 'Y坐标']}
                            labelFormatter={(label) => `点: ${label}`}
                          />
                          <Legend />
                          {cableRouteData().map((route, index) => (
                            <Line 
                              key={route.cable_id}
                              type="monotone" 
                              dataKey="y" 
                              ...
                              strokeWidth={2}
                              data={route.path.map((point: any, idx: number) => ({
                                x: point[0],
                                y: point[1]
                              }))}
                              dot={{ fill: COLORS[index % COLORS.length], r: 4 }}
                              activeDot={{ r: 6, fill: '#ffffff', stroke: COLORS[index % COLORS.length] }}
                              animationDuration={2000}
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-6">
                      {cableRouteData().map((route, index) => (
                        <motion.div
                          ...
                          className="p-3 bg-white/5 rounded-lg border border-white/10"
                        >
                          <div className="text-xs text-gray-400">{route.cable_id}</div>
                          <div className="text-lg font-bold text-cyan-400">{route.length.toFixed(2)} m</div>
                          <div className="text-xs text-gray-400">成本: {route.cost.toFixed(2)}万</div>
                        </motion.div>
                      ))}
                    </div>
                  </div>
                      ...
                      <ResponsiveContainer width="100%" height="100%">
                        ...
                        >
                          ...
                          <XAxis 
                            ...
                            stroke="#6b7280" 
                            fontSize={12}
                            label={{ value: '年份', position: 'insideBottom', offset: -5, fill: '#6b7280' }}
                          />
                          <YAxis 
                            stroke="#6b7280" 
                            fontSize={12}
                            label={{ value: '成本 (万元)', angle: -90, position: 'insideLeft', fill: '#6b7280' }}
                          />
                          <Tooltip 
                            contentStyle={{ 
                              background: 'rgba(10, 15, 26, 0.95)', 
                          ...
                          <Line 
                            ...
                            strokeWidth={2}
                            dot={{ fill: '#f59e0b', r: 4 }}
                            activeDot={{ r: 6, fill: '#ffffff', stroke: '#f59e0b' }}
                            animationDuration={2000}
                            animationBegin={900}
                          />
                          <Line 
                            ...
                            strokeWidth={3}
                            dot={{ fill: '#ef4444', r: 4 }}
                            activeDot={{ r: 6, fill: '#ffffff', stroke: '#ef4444' }}
                            animationDuration={2000}
                            animationBegin={1200}
                          />
                        </RechartsLineChart>
                        ...
                        <RechartsLineChart data={convergenceData()} margin={{ top: 20, right: 80, left: 20, bottom: 20 }}>
                          ...
                          <Tooltip 
                            ...
                            formatter={(value: any, name: any) => {
                              ...
                              return [value.toFixed(4), (labels as Record<string, string>)[name as string] || name];
                            }}
                            labelFormatter={(value) => `迭代 ${value}`}
                          />
                          <Legend verticalAlign="top" height={36} />
                  ...
                  >
                    ...
                    <div className="h-96">
                      <div className="flex justify-center items-center h-full">
                        <img 
                          src={`/algorithm/static/pareto_front_${currentInstanceId}.png`} 
                          alt="帕累托前沿图" 
                          className="max-w-full max-h-full object-contain rounded-lg shadow-lg"
                          onError={(e) => {
                            (e.target as HTMLImageElement).src = `/algorithm/static/pareto_front_r1.png`;
                          }}
                        />
                      </div>
                    </div>
