...
async function request(path: string, options: RequestInit = {}, useCache: boolean = true): Promise<any> {
  // 生成请求键
  const requestKey = `${options.method || 'GET'}_${path}_${options.body ? JSON.stringify(options.body) : ''}`;
...
// 带重试机制的请求
async function requestWithRetry(path: string, options: RequestInit = {}, useCache: boolean = true, retries: number = 3): Promise<any> {
  let lastError: Error = new Error('请求失败');
  
  for (let i = 0; i < retries; i++) {
    try {
      return await request(path, options, useCache);
    } catch (error) {
...
export const apiClient = {
  get: (path: string, useCache: boolean = true) => requestWithRetry(path, { method: 'GET' }, useCache),
