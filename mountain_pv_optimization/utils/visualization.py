...
class ResultVisualizer:
    ...
    def plot_performance_radar(self, module3_output: Dict, instance_id: str):
        """
        绘制性能雷达图，展示系统的多维度指标。
        """
        metrics = module3_output.get("metrics", {})
        ...
        # 从module3_output中提取数据
        total_cost = metrics.get("total_cost", 10000)
        # 计算成本效益：使用成本优化率，假设基线成本为15000万元
        baseline_cost = 15000
        cost_efficiency = max(0, min(1, (baseline_cost - total_cost) / baseline_cost))
        system_efficiency = metrics.get("system_efficiency", 0.9)
        reliability = min(metrics.get("system_reliability", 1.0), 2.0) / 2.0  # 归一化到0-1
        constraint_satisfaction = metrics.get("constraint_satisfaction", 1.0)
        area_utilization = metrics.get("area_utilization", 1.0)
        trench_optimization = metrics.get("trench_optimization_rate", 0.5)
        ...
        # 绘制数据
        ax.plot(angles, values, 'o-', linewidth=2, label="性能指标")
        ...
        ax.set_ylim(0, 1)
        ax.set_title(f"系统性能雷达图（算例：{instance_id}")
        ax.legend(loc="upper right")
        
        # 保存图片
        save_path = os.path.join(self.save_dir, f"performance_radar_{instance_id}.png")
        plt.savefig(save_path, dpi=300, bbox_inches="tight")
        plt.close()
        print(f"性能雷达图保存至：{save_path}")

    def plot_pareto_front(self, module3_output: Dict, instance_id: str):
        """
        绘制帕累托前沿图，展示多目标优化的最优解。
        """
        # 从module3_output中提取帕累托最优解
        pareto_solutions = module3_output.get("pareto_front", [])
        
        if not pareto_solutions:
            # 如果没有帕累托解，创建一个默认解
            pareto_solutions = [{
                "cost": module3_output.get("total_cost_summary", {}).get("total_cost", 6391.84),
                "efficiency": module3_output.get("metrics", {}).get("system_efficiency", 0.9815),
                "reliability": module3_output.get("metrics", {}).get("system_reliability", 1.6076)
            }]
        
        # 提取成本、效率和可靠性数据
        costs = [sol.get("cost", 0) for sol in pareto_solutions]
        efficiencies = [sol.get("efficiency", 0) for sol in pareto_solutions]
        reliabilities = [sol.get("reliability", 0) for sol in pareto_solutions]
        ...
        ax.set_ylabel('系统效率')
        ax.set_zlabel('系统可靠性')
        ax.set_title(f'帕累托前沿（算例：{instance_id}）')
        ...
        # 保存图片
        save_path = os.path.join(self.save_dir, f"pareto_front_{instance_id}.png")
        plt.savefig(save_path, dpi=300, bbox_inches="tight")
        plt.close()
        print(f"帕累托前沿图保存至：{save_path}")
