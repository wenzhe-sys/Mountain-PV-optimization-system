...
class InstanceLoader:
    ...
    def load_instance(self, instance_id: str) -> Dict:
        ...
        for root, dirs, files in os.walk(self.base_path):
            for file in files:
                # 匹配不同难度的算例文件格式（使用精确匹配，避免r2匹配到r28）
                if (file == f"public_easy_{instance_id}.json" or 
                    file == f"public_medium_{instance_id}.json" or 
                    file == f"public_hard_{instance_id}.json"):
                    instance_file = os.path.join(root, file)
                    print(f"找到算例文件：{instance_file}")
                    break
            if instance_file:
                break
    ...
    def validate_instance(self, instance: Dict, silent: bool = False) -> bool:
        ...
        required_fields = [
            ...
            "pva_params", "equipment_params", "constraint_info"
        ]
        ...
        # 自动添加缺失的字段
        self._add_missing_fields(instance)

        # 校验核心参数取值范围
        if instance["equipment_params"]["inverter"].get("q", 320) not in [250, 320, 500]:
            if not silent:
                raise ValueError(f"逆变器容量非法，错误码E002")
            return False

        if instance["loss_params"].get("lambda", 0.4) < 0.3 or instance["loss_params"].get("lambda", 0.4) > 0.5:
            if not silent:
    ...
    def _add_missing_fields(self, instance: Dict):
        """自动添加缺失的字段，确保算例完整性"""
        # 添加缺失的 loss_params 字段
        if "loss_params" not in instance:
            instance["loss_params"] = {
                "lambda": 0.4,
                "K_segments": 3,
            ...
            print("【算例修复】自动添加缺失的 loss_params 字段")

        # 添加缺失的 dist_matrix 字段
        if "dist_matrix" not in instance:
            pva_list = instance.get("pva_list", [])
            n = len(pva_list)
            # 计算曼哈顿距离矩阵
            dist_matrix = []
            for i in range(n):
                ...
                for j in range(n):
                    if i == j:
                        ...
                        dx = abs(pva_list[i]["x"] - pva_list[j]["x"])
                        dy = abs(pva_list[i]["y"] - pva_list[j]["y"])
                        row.append(dx + dy)  # 曼哈顿距离
                dist_matrix.append(row)
            instance["dist_matrix"] = dist_matrix
            print("【算例修复】自动添加缺失的 dist_matrix 字段")

        # 添加缺失的 terrain_data 字段
        if "terrain_data" not in instance:
            instance["terrain_data"] = {
                "grid_size": 10,
                "slope_matrix": [[0.0 for _ in range(10)] for _ in range(10)],
                "buildable_matrix": [[True for _ in range(10)] for _ in range(10)],
                "rows": 10,
                "cols": 10
            }
            print("【算例修复】自动添加缺失的 terrain_data 字段")

        # 完善 terrain_data 中的 slope_matrix
        if "terrain_data" in instance:
            terrain_data = instance["terrain_data"]
            if "slope_matrix" not in terrain_data:
                grid_size = terrain_data.get("grid_size", 10)
                rows = terrain_data.get("rows", 10)
                cols = terrain_data.get("cols", 10)
                terrain_data["slope_matrix"] = [[0.0 for _ in range(cols)] for _ in range(rows)]
                print("【算例修复】自动添加缺失的 terrain_data.slope_matrix 字段")

        # 完善 equipment_params 字段
        if "equipment_params" in instance:
            if "inverter" in instance["equipment_params"]:
                if "q" not in instance["equipment_params"]["inverter"]:
                    instance["equipment_params"]["inverter"]["q"] = 320.0
                if "r" not in instance["equipment_params"]["inverter"]:
                    instance["equipment_params"]["inverter"]["r"] = 0.85
            if "transformer" not in instance["equipment_params"]:
                instance["equipment_params"]["transformer"] = {
                    "Q_box_options": [1600, 3200],
                    "c_box": {"1600": 30.0, "3200": 50.0},
                    "c_install_box": {"1600": 5.0, "3200": 3.0}
                }
            if "cable" not in instance["equipment_params"]:
                instance["equipment_params"]["cable"] = {
                    "c1": 15.0,
                    "c2": 35.0,
                    "c3": 200.0,
                    "rho": 1.72e-08,
            ...
            if "substation" not in instance["equipment_params"]:
                instance["equipment_params"]["substation"] = {
                    "Q_substation": 50,
                    "coord": [35.0, 35.0]
                }

        # 完善 pva_params 字段
        if "pva_params" in instance:
            if "min_panels_per_zone" not in instance["pva_params"]:
                instance["pva_params"]["min_panels_per_zone"] = 18
            if "max_panels_per_zone" not in instance["pva_params"]:
                instance["pva_params"]["max_panels_per_zone"] = 26
            if "b" not in instance["pva_params"]:
                instance["pva_params"]["b"] = 3.0
