...
def build_adjacency_graph(pva_list: List[Dict], grid_size: float = 10.0, terrain_data: Dict = None) -> nx.Graph:
    """
    从面板列表构建网格邻接图（四邻域连接）。
    ...
    Args:
        ...
        terrain_data: 地形数据，包含 slope_matrix 和 buildable_matrix
    ...
    for pva in pva_list:
        ...
        if terrain_data:
            # 检查该位置的坡度和可建设性
            if "slope_matrix" in terrain_data and "buildable_matrix" in terrain_data:
                slope_matrix = terrain_data["slope_matrix"]
                buildable_matrix = terrain_data["buildable_matrix"]
                
                # 确保坐标在矩阵范围内
                if 0 <= row < len(slope_matrix) and 0 <= col < len(slope_matrix[0]):
                    slope = slope_matrix[row][col]
                if 0 <= row < len(buildable_matrix) and 0 <= col < len(buildable_matrix[0]):
                    buildable = buildable_matrix[row][col]
        ...
        graph.add_node(panel_id, x=x, y=y, row=row, col=col,
                       ...
                       slope=slope, buildable=buildable)
    ...
    for row, panels in row_groups.items():
        ...
        for i in range(len(panels) - 1):
            ...
            if col_gap <= 5 and not graph.has_edge(id1, id2):
                ...
                # 考虑地形因素：如果两个面板之间的区域不可建设或坡度太大，不建立边
                can_connect = True
                if terrain_data and "buildable_matrix" in terrain_data:
                    buildable_matrix = terrain_data["buildable_matrix"]
                    # 检查中间区域的可建设性
                    min_col, max_col = min(col1, col2), max(col1, col2)
                    for c in range(min_col, max_col + 1):
                        if 0 <= row < len(buildable_matrix) and 0 <= c < len(buildable_matrix[0]):
                            if not buildable_matrix[row][c]:
                                can_connect = False
                                break
            ...
            if row_gap <= 5 and not graph.has_edge(id1, id2):
                ...
                if terrain_data and "buildable_matrix" in terrain_data:
                    ...
                    for r in range(min_row, max_row + 1):
                        if 0 <= r < len(buildable_matrix) and 0 <= col < len(buildable_matrix[0]):
                            if not buildable_matrix[r][col]:
                                can_connect = False
                                break
