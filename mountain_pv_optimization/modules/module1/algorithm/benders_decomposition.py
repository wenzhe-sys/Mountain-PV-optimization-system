...
class BendersDecomposition:
    ...
    def calculate_perimeter(self) -> List[float]:
        ...
        if not hasattr(self, 'grid_size'):
            self.grid_size = self.pva_list[0].get("grid_coord", [10.0])[0]
        grid_size = self.grid_size
        
        # 优化：预先计算每个节点的邻居，避免重复计算
        if not hasattr(self, 'node_neighbors'):
            self.node_neighbors = {i: list(self.graph.neighbors(i)) for i in range(self.n_nodes)}
        
        for k in range(self.p):
            if self.y_m[k]:
                boundary_count = 0
                # 优化：使用预先计算的邻居列表，减少遍历次数
                for i in range(self.n_nodes):
                    if self.sigma_ik[i, k]:
                        # 检查当前节点的邻居是否属于其他分区
                        for j in self.node_neighbors[i]:
                            if self.sigma_ik[j, k] != self.sigma_ik[i, k]:
                                boundary_count += 1
                                break  # 每个节点只计数一次边界
    ...
    def optimize(self) -> Dict:
        ...
        # 初始化
        if not hasattr(self, 'cutting_solver'):
            self.cutting_solver = CuttingMasterProblem(D=self.D, t_l_options=self.t_l_options)
        cutting_solver = self.cutting_solver
        # 清空之前的割平面
        cutting_solver.cuts = []
