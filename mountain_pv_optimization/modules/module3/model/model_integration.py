...
class IntegrationOptimizationModel:
    def __init__(self, instance_path: str, module2_output_path: str, module1_output: Dict = None, max_iter: int = 20):
        ...
        self.rl_optimizer = RLIntegrationOptimizer(self.instance_data, self.module2_output.copy(), self.module1_output, max_iter=max_iter)
        
        # 使用相对路径构建结果保存路径（保存到根目录的results文件夹）
        current_dir = os.path.dirname(os.path.abspath(__file__))
        project_root = os.path.dirname(os.path.dirname(os.path.dirname(current_dir)))  # 上三级目录（模块三目录的父目录）
        self.results_path = os.path.join(project_root, "data", "results", "module3")
        os.makedirs(self.results_path, exist_ok=True)

    def load_instance(self) -> Dict:
        # 从文件路径中提取instance_id，处理所有难度级别
        filename = os.path.basename(self.instance_path)
        if filename.startswith("public_easy_"):
            instance_id = filename[len("public_easy_"):-len(".json")]
        elif filename.startswith("public_medium_"):
            instance_id = filename[len("public_medium_"):-len(".json")]
        elif filename.startswith("public_hard_"):
            instance_id = filename[len("public_hard_"):-len(".json")]
        else:
            # 备用：直接替换
            instance_id = filename.replace("public_easy_", "").replace("public_medium_", "").replace("public_hard_", "").replace(".json", "")
    ...
    def run(self) -> Dict:
        ...
        module3_output = {
            ...
            "total_cost_summary": {
                ...
                "civil_cost": float(integration_result["total_cost_summary"].get("civil_cost", 0.0)),
                "operation_cost": float(integration_result["total_cost_summary"].get("operation_cost", 0.0)),
                "lcoe": float(integration_result["total_cost_summary"].get("lcoe", 0.0)),
                "cost_breakdown": {
                    ...
                    "civil": float(integration_result["total_cost_summary"].get("civil_cost", 0.0)),
                    "operation": float(integration_result["total_cost_summary"].get("operation_cost", 0.0))
                }
            },
            "optimized_params": {
            ...
            "algorithm_info": {
                ...
                "iterations": integration_result.get("iterations", 0),
                "convergence": integration_result.get("convergence", False)
            }
        }
        ...
        print(f"  - 算法迭代次数：{integration_result.get('iterations', 0)}")
